...
global {
	//My population
	int S_people <- 70000000 min: 50 max:100000000; //Number of sc=usceptible individuals
	int I_people <- 1 min: 1 max: 541583; // infectious people (active TB)
...
species math_TB {
	...
	/*equation SLeLfITKR1R2 { 
			...
			diff(I,t)= (w*Lf + q*Le - (r1 + gamma + beta + sigma + v + mu1 + mu2)*I + alpha*lambda*r*R1*I + alpha*lambda*r*R2*I + alpha*lambda*(1-p)*S*I/N);
	...
	equation SLeLfITKR1R2 { 
		    diff(S,t) = (Gamma*N - alpha*lambda*p *S * I/N - alpha*lambda*(1-p)*S*I/N - mu1*S);
		    diff(Le,t) = (alpha*lambda*p *S * I/N + alpha*lambda*r*I*(R1 + R2) + r1*I + r2*T + r3*K - (mu1 + h + q+ g1+ g2)*Le);
			diff(Lf,t) = (h*Le - (mu1 + w + k1 + k2)*Lf);
    	...
    	reflex solving {solve SLeLfITKR1R2 method: rk4 step: 0.0007;
...
experiment Simulation_Math_TB type: gui {
	...
	parameter 'Mortality linked to TB: mu1' type: float var: mu1 <- 0.04 category: "Parameters";//rate of mortality related to TB infection  
	parameter 'Mortality not linked to TB: mu2' type: float var: mu2 <- 0.01045  category: "Parameters"; //rate of mortality not related to TB infection 0.0003
	...
	parameter 'Rate of contact' type: float var:alpha <-0.72  category: "Parameters"; //rate of contact
	parameter 'Rate of transmission: lambda' type: float var: lambda <- 0.150 category: "Parameters"; // rate of transmission
	parameter 'Proportion of people: p (S-> Le & I)' type: float var: p <-0.9  category: "Parameters"; // proportion of people
